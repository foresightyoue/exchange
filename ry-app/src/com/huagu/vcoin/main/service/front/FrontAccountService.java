...
public class FrontAccountService {
    ...
    public boolean updateWithdrawCNY(double withdrawBanlance, Fuser fuser, FbankinfoWithdraw fbankinfoWithdraw)
        ...
        try {
            ...
            Fwithdrawfees ffees = this.withdrawfeesDAO.findFfee(fvirtualwallet.getFvirtualcointype().getFid(),
    ...
    public void updateCancelWithdrawCny(Fcapitaloperation fcapitaloperation, Fuser fuser) {
        try {
            ...
            fcapitaloperation.setfLastUpdateTime(Utils.getTimestamp());
            Fvirtualwallet fvirtualwallet = this.fvirtualwalletDAO.findWallet(fuser.getFid());
            fvirtualwallet.setFfrozen(
                    fvirtualwallet.getFfrozen() - fcapitaloperation.getFfees() - fcapitaloperation.getFamount());
            fvirtualwallet.setFtotal(
                    fvirtualwallet.getFtotal() + fcapitaloperation.getFfees() + fcapitaloperation.getFamount());
            this.fvirtualwalletDAO.attachDirty(fvirtualwallet);
            this.fcapitaloperationDAO.attachDirty(fcapitaloperation);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public int getTodayVirtualCoinWithdrawTimes(Fuser fuser, int coinId) {
        return this.fvirtualcaptualoperationDAO.getTodayVirtualCoinWithdrawTimes(fuser, coinId);
    }
}
