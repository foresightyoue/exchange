...
import com.huagu.vcoin.main.Enum.TransportlogStatusEnum;
...
import com.huagu.vcoin.main.service.admin.TransportlogService;
...
public class TransportlogController extends BaseController {
    ...
    public ModelAndView Index() throws Exception {
        ...
        List<Ftransportlog> list = this.transportlogService.list((currentPage - 1) * numPerPage, numPerPage,
                filter + "", true);
    ...
    public ModelAndView transportlogAudit() throws Exception {
        ...
        Fvirtualwallet fvirtualwallet = this.frontUserService.findVirtualWalletByUser(log.getFuser().getFid(),
                log.getFvirtualcointype().getFid());
        Fuser user = this.frontUserService.findById(Integer.parseInt(log.getFaddress()));
        Fvirtualwallet fw2 = this.frontUserService.findVirtualWalletByUser(user.getFid(),
                log.getFvirtualcointype().getFid());
