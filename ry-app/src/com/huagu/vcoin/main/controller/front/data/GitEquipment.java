...
public class GitEquipment {

    // /\b 是单词边界(连着的两个(字母字符 与 非字母字符)之间的逻辑上的间隔),
    //
    // // 字符串在编译时会被转码一次,所以是 "\\b"
    // // \B 是单词内部逻辑间隔(连着的两个字母字符之间的逻辑上的间隔)
    public static String phoneReg = "\\b(ip(hone|od)|android|opera m(ob|in)i" + "|windows (phone|ce)|blackberry"
     ...
     */
    protected static boolean check(String userAgent) {
        if (null == userAgent) {
            userAgent = "";
        }
        // 匹配
     ...
     * @return
     */
    protected static String getDeviceName(String userAgent) {
        if (null == userAgent) {
            userAgent = "";
        }
        // 匹配
