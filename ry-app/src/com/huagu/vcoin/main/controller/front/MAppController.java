...
public class MAppController extends BaseController {
    ...
    public ModelAndView top5paihang(HttpServletRequest request) throws Exception {
        JspPage modelAndView = new JspPage(request);

        List<Ftrademapping> ftrademappings = this.utilsService.list(0, 0, " where fstatus=? order by fid asc ", false,
        ...
        for (int i = 0; i < ftrademappings.size(); i++) {
            ...
            double s1 = this.redisUtil.getDouble(RedisConstant.getstart24Price(ftrademapping.getFid()));
            double s7 = -8888d;
            List<Ftradehistory> ftradehistorys = (List<Ftradehistory>) constantMap.get("tradehistory");
            if (ftradehistorys != null) {
            ...
            double price = this.redisUtil.getDouble(RedisConstant.getLatestDealPrizeKey(ftrademapping.getFid()));
            double last = 0d;
            double last7 = 0d;
            try {
                last = s1 != 0 ?  Utils.getDouble((price - s1) / s1 * 100, 2) : 0;
            } catch (Exception e) {
                e.printStackTrace();
            }
            try {
            ...
            ftrademapping.setFvirtualcointypeByFvirtualcointype2(this.frontVirtualCoinService
                    .findFvirtualCoinById(ftrademapping.getFvirtualcointypeByFvirtualcointype2().getFid()));
            ftrademapping.setRose(last);
            ftrademapping.setRose7(last7);
            ftrademapping.setTotal24(
                    Utils.getDouble(this.redisUtil.getDouble(RedisConstant.getTotal(ftrademapping.getFid())), 2));
            ftrademapping.setTotalCny24(
                    Utils.getDouble(this.redisUtil.getDouble(RedisConstant.get24Total(ftrademapping.getFid())), 2));
        ...
        modelAndView.setJspFile("/front/app/top5paihangbang");
        return modelAndView;
    }
    ...
    public ModelAndView tradePwd(HttpServletRequest request) throws Exception {
        ...
        modelAndView.setJspFile("/front/app/security/tradePwd");
        return modelAndView;
    }
    ...
    public ModelAndView realId(HttpServletRequest request) throws Exception {
        ...
        modelAndView.setJspFile("/front/app/security/realId");
    ...
    public ModelAndView regSuc(HttpServletRequest request) throws Exception {
        ...
        modelAndView.setJspFile("/front/app/security/regsuc");
        return modelAndView;
    }
    ...
    public ModelAndView resetPwd(HttpServletRequest request) throws Exception {
        ...
        modelAndView.setJspFile("/front/app/user/resetPwd1");
        return modelAndView;
    }

    @RequestMapping("/m/validate/resetPwd2")
    public ModelAndView resetPwd2(HttpServletRequest request) throws Exception {
        JspPage modelAndView = new JspPage(request);

        modelAndView.setJspFile("/front/app/user/resetPwd2");
        return modelAndView;
    }

    @RequestMapping("/m/validate/resetPwd3")
    public ModelAndView resetPwd3(HttpServletRequest request) throws Exception {
        JspPage modelAndView = new JspPage(request);

        modelAndView.setJspFile("/front/app/user/resetPwd3");
        return modelAndView;
    }

    @RequestMapping("/m/reg-notice")
    public ModelAndView regNotice(HttpServletRequest request) throws Exception {
        JspPage modelAndView = new JspPage(request);

        modelAndView.setJspFile("/front/app/user/reg-notice");
        return modelAndView;
    }

    @RequestMapping("/m/realIdentity")
    public ModelAndView realIdentity(HttpServletRequest request) {
        JspPage jspPage = new JspPage(request);
        String flag = request.getParameter("flag");
        if (!StringUtils.isEmpty(flag)) {
            jspPage.addObject("flag", flag);
        }
        Fuser fuser = this.frontUserService.findById(GetCurrentUser(request).getFid());
        jspPage.addObject("fuser", fuser);
        if (fuser.isFhasImgValidate() && fuser.isFpostBankValidate() || fuser.isFhasBankValidate()) {
            jspPage.setJspFile("front/user/realCertification2");
        } else {
            jspPage.setJspFile("/front/app/security/realIdentity");
            // jspPage.setJspFile("/front/app/security/realbackId");
        }
    ...
    @RequestMapping("/m/realbackId")
    public ModelAndView realbackId(HttpServletRequest request) {
        JspPage page = new JspPage(request);
        Fuser fuser = this.frontUserService.findById(GetCurrentUser(request).getFid());
        this.SetSession(fuser, request);
        page.addObject("fuser", fuser);
        page.setJspFile("/front/app/security/realbackId");
        return page;
