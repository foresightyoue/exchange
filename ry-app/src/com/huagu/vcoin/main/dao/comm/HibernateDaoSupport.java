...
package com.huagu.vcoin.main.dao.comm;
...
public abstract class HibernateDaoSupport extends DaoSupport {
    ...
    public List findByParam(int firstResult, int maxResults, String filter, boolean isFY, Class c, Object... param) {
        List list = null;
        log.debug("finding " + c.getName() + " instance with filter");
        try {
            String queryString = "from " + c.getName() + " " + filter;
            ...
            log.error("find by " + c.getName() + " name failed", re);
            throw re;
        }
        return list;
    }

    public List findByParam(int firstResult, int maxResults, String filter, boolean isFY, Class c) {
        List list = null;
        log.debug("finding " + c.getName() + " instance with filter");
        try {
            String queryString = "from " + c.getName() + " " + filter;
    ...
    public int findByParamCount(String filter, Class c) {
        List list = null;
        try {
            String queryString = "select count(*) from " + c.getName() + " " + filter;
            Query queryObject = getSession().createQuery(queryString);
    ...
    public List findKlineQuery(Map<String, Object> map, Date datefrom, Date dateto) {
        List list = null;
      try {
            list = getSession().createCriteria(Fperiod.class).add(
                    Restrictions.eq("ftrademapping.fid", map.get("symbol")))
                    .add(Restrictions.eq("frequency", map.get("resolution")))
                    .add(Restrictions.between("ftime", datefrom, dateto))
                    .addOrder(Order.asc("ftime")).list();
        } catch (RuntimeException re) {
            log.error("find by findKlineQuery failed", re);
            throw re;
        }
        return list;
    }
    public List findHQLKline(int firstResult, int maxResults, String filter, boolean isFY, Class c, Object... param) {
        ...
        try {
            ...
            log.error("find by findHQL failed", re);
            throw re;
        }
        return list;
    }
