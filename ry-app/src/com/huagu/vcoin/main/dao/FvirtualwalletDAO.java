...
public class FvirtualwalletDAO extends HibernateDaoSupport {
    ...
    public Fvirtualwallet findWallet(int userId) {
        ...
        if (list.size() == 1) {
            ...
            fixUserById(userId); // 自动修复钱包错误，然后再次执行
            ...
            if (list2.size() != 0)
                throw new RuntimeException(String.format("钱包数据异常, 该用户(userId=%d)钱包记录数为：%d", userId, list2.size()));
            return list2.get(0);
        } else {
            throw new RuntimeException(String.format("钱包数据异常, 该用户(userId=%d)钱包记录数为：%d", userId, list.size()));
        }
    }
    ...
    protected void fixUserById(int userId) {
        try (Mysql mysql = new Mysql()) {
            ...
            ds1.open();
            if (ds1.eof()) {
                throw new RuntimeException("没有找到默认的法币类别，请于后台修复！");
            }
            // 打开所有的币种
            ds1.close();
            ds1.clear();
            ...
            if (ds2.size() != ds1.size()) {
                log.warn(String.format("币种类别与钱包记录数必须一致(t=%d, w=%d)！", ds1.size(), ds2.size()));
                ...
                while (ds3.fetch()) {
                    ...
                    ds2.post();
                    log.warn(String.format("用户(userId=%d)钱包缺少币种为%d的记录，已修复", userId, ds3.getInt("fId")));
                }
            }
        }
    }
