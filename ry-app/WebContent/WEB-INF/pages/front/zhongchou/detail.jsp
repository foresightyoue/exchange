...
<body>
	...
	<div class="container-full main-con" style="margin-top: 10px;">
					...
					<div class="crowd_con clear">
						<div class="crowd_con_l " style="width: 740px;height: 320px;">
							<div class="crowd_bi clear">
								...
								<div class="crowd_hui crowd_hui_${fsubscription.fstatus=='进行中'?'green':'red' }">
									<em></em>${fsubscription.fstatus}
								</div>
								<div style="float: right;width: 100px;height: 30px;border-radius: 2px;margin-top: 14px;
							     margin-right: 10px;line-height: 28px;padding: 2px 8px;color:white;text-align: center;"><a href="${fsubscription.fbaipi}" target="_blank">查看白皮书</a></div>
							</div>
							<ul class="crowd_subscribe clear">
								<li style="list-style-type:none;">
								<p>总量(${fsubscription.fvirtualcointype.fShortName })</p>
								<span class="red"> <fmt:formatNumber value="${fsubscription.ftotal }" pattern="##.##" maxIntegerDigits="20" maxFractionDigits="0"/></span></li>
								<li style="list-style-type:none;">
								<p>剩余数量(${fsubscription.fvirtualcointype.fShortName })</p>
							...
							<div class="crowd_time">
								<div>
								<c:choose>
								<c:when test="${fsubscription.fstatus=='进行中'}">
								　　　<p class="crowd_con_lp" id="cdstatus">距离支持结束还有：</p>
								...
								　　　<p class="crowd_con_lp" id="cdstatus">距离支持开始还有：</p>
									<div class="crowd_time_all">
										<div id="t_d">
											<em>天</em>
										</div>
								...
								<c:otherwise>
								   <p style="text-align: center; font-size: 22px; letter-spacing: 1px; line-height: 70px;">支持成功</p>
								</c:otherwise>
								</c:choose>
								</div>
							</div>
							...
							<div class="crowd_my">
									...
									<div class="login_text zin90 clear">
										<span id="subCrowdNum" class="crowd_num">-</span> <input
											...
											placeholder="请输入您要支持的数量" /> <span id="addCrowdNum"
									...
									<c:choose>
									<c:when test="${fsubscription.fstatus=='进行中'}">
										<input type="button" value="立即支持" onclick="joincrowd()" class="login_button_x"/>
									</c:when>
									<c:otherwise>
										<input style="background: #aaa" value="支持${fsubscription.fstatus}" onclick="return false" type="button">
									</c:otherwise>	
									</c:choose>	
									</div>
									<div class="login_button">
										<a href="/crowd/logs.html" target="_blank" class="right crowd_look orange">查看我的支持记录</a>
									</div>
							</div>
						</div>
					</div>
