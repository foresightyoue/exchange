...
<div class="col-xs-12 article-main">
	  <div class="cols-xs-12 text-left">
			<p>
            <a href="${oss_url}">首页</a> / 
            <a href="${oss_url}/service/ourService.html?id=${atype.fid }">${atype.fname }</a>
			</p>
		</div>
      <ul class="list-group">
      <c:forEach items="${farticles }" var="v">
      <li class="list-group-item">
          <a class="pull-left" href="/service/article.html?id=${v.fid }">${v.ftitle_short } </a>
            <span class="pull-right"><fmt:formatDate value="${v.fcreateDate}" pattern="yyyy-MM-dd HH:mm:ss"/></span>
           </li>
      </c:forEach>     
