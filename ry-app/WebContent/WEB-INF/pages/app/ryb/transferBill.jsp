...
<body>
    ...
    <section id="wallet" v-cloak>
        <div class='selectBox'>
            <!-- <select id="type" name="type" v-on:change="request" v-model='coin' class='firstSelect'>
                <option v-for="(value, key,i) in coinType" v-bind:value="key">{{value}}</option>
            </select>
            <select v-on:change="request" v-model='type' class='secondSelect'>
                <option v-for="(value, key,i) in capitalType" v-bind:value="key">{{value}}</option>
            </select> -->
            <div style="width: 100%;">
        ...
        <div>
            <div v-for="(item,i) in bills" class='itemBox'>
                ...
                <p><span>币种类型：</span><span>{{item.accountId}}</span></p>
                <p><span>数&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;量：</span><span>{{item.currency}}</span></p>
                <p><span>创建时间：</span><span>{{item.createDate}}</span></p>
                <p><span>流&nbsp;&nbsp;水&nbsp;号：</span><span class='waterNum'>{{item.tid}}</span> <span class="pRight right2" v-on:click="copy(item.tid,'.right2')">复制</span></p>
            ...
            request: function () {
                ...
                if (this.date != "") {
                    $.post("/api/trading/getjson2.html", {
                        "search": this.date,
                        "uri": "/api/app/getTransferMallRecord"
                    }, function (data) {
                        console.log(data);
