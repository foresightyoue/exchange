...
<div class="header">
    ...
    <div class="head_title font15">我的广告订单</div>
            ...
            <li style="margin-top: 0;">
                <a href="javascript: void (0);">
                    ...
                    <div class="flexLayout ad_time">
                        <div class="font12 text-gray">创建时间：<fmt:formatDate value="${otcOrder.fCreateTime }" pattern="yyyy-MM-dd HH:mm"></fmt:formatDate></div>
                    </div>
                    <div class="flexLayout ad_time">
                    ...
                    <div class="flexLayout ad_time">
                        ...
                        <div class="font13">交易类型：
                            <c:if test="${otcOrder.fType == '1' }">出售</c:if>
                            <c:if test="${otcOrder.fType == '2' }">购买</c:if>
                        </div>
                    </div>
                    ...
                    <div class="tr pt_10 operation_btn2">
                        ...
                        <c:if test="${otcOrder.fType == '1' }">
                            <a href="/m/otc/payOrder.html?coinType=${coin_id}&orderId=${otcOrder.fOrderId }&type=w3c&eq=w3c&fw=1" class="table_btn font13 mt_0">查看支付情况</a>
                        </c:if>
                        <!-- 如果订单是买入 -->
                        <c:if test="${otcOrder.fType == '2' }">
                        <!-- 如果订单状态为1已下单，显示按钮去支付和取消订单 -->
                            <c:if test="${otcOrder.fTrade_Status == '1'}">
                                <a href="/m/otc/payOrder.html?coinType=${coin_id}&orderId=${otcOrder.fOrderId }&fw=1" class="table_btn font13 mt_0">去支付</a>
                                <a href="/m/otc/cancleOrder.html?coinType=${coin_id}&orderId=${otcOrder.fOrderId }&fw=1" class="table_btn font13 mt_0 mr_10">取消订单</a>
                            </c:if>
                           <!-- 如果订单状态为2待验证，显示按钮查看订单详情 -->
                            <c:if test="${otcOrder.fTrade_Status == '2'}">
                                <a href="/m/otc/appeal.html?coinType=${coin_id}&orderId=${otcOrder.fOrderId }&fw=1" class="table_btn font13 mt_0">申诉</a>
                                <a href="/m/otc/payOrder.html?coinType=${coin_id}&orderId=${otcOrder.fOrderId }&fw=1" class="table_btn font13 mt_0 mr_10">查看详情</a>
                            </c:if>
                        </c:if>
                    </div>
                </a>
            ...
            <div class="bk-pageTit" id="exchangeRecord">
                 ...
                 <div class="table-responsive" id="ing" style="display:block;">
                      <c:forEach var="otcOrder" items="${orderListIng}">
                         <div>
                            <ul>
                               <li>订单号<span>${otcOrder.fOrderId }</span>&nbsp;&nbsp;&nbsp;&nbsp;
                                    <span>
                                       <c:if test="${otcOrder.fTrade_Status == '1' }">待支付</c:if>
                                       <c:if test="${otcOrder.fTrade_Status == '2' }">待确认</c:if>
                                       <c:if test="${otcOrder.fTrade_Status == '5' }">申诉中</c:if>
                               ...
                               <li>创建时间:<fmt:formatDate value="${otcOrder.fCreateTime }" pattern="yyyy-MM-dd HH:mm"></fmt:formatDate></li>
                               ...
                               <li>
                                                                                     交易对象:<span>${otcOrder.floginName }</span>
                               </li>
                               <li>
                                                                                     单价:<span>${otcOrder.fTrade_SumMoney }</span>
                               </li>
                               <li>数量:<span>${otcOrder.fTrade_Count }</span></li>
                               <li>
                                    <!-- 如果订单是卖出 -->
                                    <c:if test="${otcOrder.fType == '1' }">
                                             <a href="/m/otc/payOrder.html?orderId=${otcOrder.fOrderId }&type=w3c&eq=w3c&fw=1" style="padding: 5px 10px;background-color: #1db3b4;color: white;border-radius: 4px;float: left;">查看支付情况</a>
                                    </c:if>
                                    <!-- 如果订单是买入 -->
                                    <c:if test="${otcOrder.fType == '2' }">
                                    <!-- 如果订单状态为1已下单，显示按钮去支付和取消订单 -->
                                       <c:if test="${otcOrder.fTrade_Status == '1'}">
                                          <a href="/m/otc/payOrder.html?orderId=${otcOrder.fOrderId }&fw=1"style="padding: 5px 10px;background-color: #1db3b4;color: white;border-radius: 4px;margin-bottom: 5px;float: left">去支付</a>
                                          <a href="/m/otc/cancleOrder.html?orderId=${otcOrder.fOrderId }&fw=1" style="padding: 5px 10px;background-color: #1db3b4;color: white;border-radius: 4px;float: left;">取消订单</a>
                                       </c:if>
                                       <!-- 如果订单状态为2待验证，显示按钮查看订单详情 -->
                                       <c:if test="${otcOrder.fTrade_Status == '2'}">
                                           <a href="/m/otc/appeal.html?orderId=${otcOrder.fOrderId }&fw=1"style="padding: 5px 10px;background-color: #1db3b4;color: white;border-radius: 4px;margin-bottom: 5px;float: left">申诉</a>
                                           <a href="/m/otc/payOrder.html?orderId=${otcOrder.fOrderId }&fw=1"style="padding: 5px 10px;background-color: #1db3b4;color: white;border-radius: 4px;margin-bottom: 5px;float: left">查看详情</a>
                                       </c:if>
                                    </c:if>
                               </li>
                            </ul>
                        ...
                        <c:if test="${page_countsIng >= 1}">
                            <div class="foot-page" align="center">
                                <a href="/otc/orderShellList.html?currentPage=1">首页</a> <a
                                    href="/otc/orderShellList.html?currentPage=${currentPage-1 }">上一页</a> <a
                                    href="/otc/orderShellList.html?currentPage=${currentPage+1 }">下一页</a> <a
                                    href="/otc/orderShellList.html?currentPage=${page_finalIng }">尾页</a>
                       ...
                       <c:if test="${page_countsOver >= 1}">
                            <div class="foot-page" align="center">
                                <a href="/otc/orderShellList.html?currentPage=1">首页</a> <a
                                    href="/otc/orderShellList.html?currentPage=${currentPage-1 }">上一页</a> <a
                                    href="/otc/orderShellList.html?currentPage=${currentPage+1 }">下一页</a> <a
                                    href="/otc/orderShellList.html?currentPage=${page_finalOver }">尾页</a>
