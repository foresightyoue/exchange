	...
	<table class="table" width="150%" layoutH="138">
		<thead>
			<tr>
				...
				<th width="60" orderField="isrobotType"
                    <c:if test='${param.orderField == "isrobotType"}'> class="${param.orderDirection}"  </c:if>>交易类别</th>
				...
				<th width="60" orderField="fcount"
					<c:if test='${param.orderField == "fcount"}'> class="${param.orderDirection}"</c:if>>数量	
				</th>
		...
		<tbody>
			<c:forEach items="${entrustList}" var="entrust" varStatus="num">
				<tr target="sid_user" rel="${entrust.fid}">
					...
					<c:if test="${entrust.isRobotType == '1' }">
					   <td>普通用户交易</td>
					</c:if>
					<c:if test="${entrust.isRobotType == '0' }">
                       <td>机器人交易</td>
                    </c:if>
                    <c:if test="${entrust.isRobotType == '-1' }">
                       <td>全部</td>
					...
					<td><fmt:formatDate value="${entrust.flastUpdatTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
					<td><fmt:formatDate value="${entrust.fcreateTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
	...
	<div class="panelBar">
		<div class="pages">
			<span>总共: ${totalCount}条；
			 查询区间总成交数量：<font color="red"><fmt:formatNumber value="${totalAmt }" pattern="##.######" maxIntegerDigits="15" maxFractionDigits="6"/></font>; 
			 查询区间总成交金额：<font color="red"><fmt:formatNumber value="${totalQty }" pattern="##.######" maxIntegerDigits="15" maxFractionDigits="6"/></font>;
			  查询区间总手续费：<font color="red"><fmt:formatNumber value="${fees }" pattern="##.######" maxIntegerDigits="15" maxFractionDigits="6"/></font>;</span>
		</div>
