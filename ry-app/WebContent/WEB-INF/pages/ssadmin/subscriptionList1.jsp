		...
		<thead>
			<tr>
				...
				<th width="60" orderField="fprice"
					<c:if test='${param.orderField == "fprice" }'> class="${param.orderDirection}"  </c:if>>兑换比例</th>
		...
		<tbody>
			<c:forEach items="${subscriptionList1}" var="subscription"
				...
				<tr target="sid_user" rel="${subscription.fid}">
					...
					<td><fmt:formatDate value="${subscription.fbuyTimes}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
					<td><fmt:formatDate value="${subscription.fbeginTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
					<td><fmt:formatDate value="${subscription.fendTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
					<td><fmt:formatDate value="${subscription.fcreateTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
