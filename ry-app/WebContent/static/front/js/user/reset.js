var reset = {
	findPassPhone : function(btnele) {
		...
		if (loginName == "") {
			util.showerrortips("reset-errortips", language["reset.error.tips.4"]);
			return;
		}
...
$(function() {
	...
	$(".btn-sendmsg").on("click", function() {
		if (this.id == "reset-sendmessage") {
			var areacode = $("#reset-areaCode").val();
			var phone = $("#reset-phone").val();
			var sendVoice = $("#sendVoice").val();
			msg.sendMsgCode($(this).data().msgtype, $(this).data().tipsid, this.id, areacode, phone,$("#reset-imgcode").val(),sendVoice);
		} else {
