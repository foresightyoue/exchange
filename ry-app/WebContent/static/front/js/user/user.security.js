var security = {
            ...
            if (data.code == 0) {
                if (flag) {
                    ...
                    window.setTimeout(function() {
                        if (!isPc()) {
                            window.location.href = '/m/security.html';
                        } else {
    ...
    submitValidatePhoneForm : function(isUpdate) {
        ...
        jQuery.post(url, param, function(result) {
            ...
            if (result.code == 0) {
                ...
                $("#unbindphone-sendmessage").closest('.form-group').show();
            }
        }, "json");
    },
    ...
    submitlookbindGoogleForm : function() {
        var msgcode = document.getElementById("unbindgoogle-msgcode").value;
        var desc = '';
        if (msgcode) {
            if (msgcode.indexOf(" ") > -1 || msgcode.length != 6
                    || !/^[0-9]{6}$/.test(msgcode)) {
                desc = language["comm.error.tips.124"];
        ...
        var url = "/user/getGoogleCheck.html?random="
                + Math.round(Math.random() * 100);
        var param = {
        ...
        jQuery.post(url, param, function(result) {
            ...
            if (result.code == 0) {
                ...
                $("#unbindgoogle-msgcode").closest('.form-group').show();
            }
        }, "json");
        ...
        if (totpCode.length < 5) {
            return;
        }

        util.showerrortips('unbindgoogle-errortips', "");
    ...
    submitPwdForm : function(pwdType, istrade) {
        ...
        var phoneCode = "";
        var totpCode = "";
        if (!$("#" + phoneCodeEle).is(":hidden")
                && document.getElementById(phoneCodeEle) != null) {
        ...
        // return;
        // }
        var url = "/user/modifyPwd.html?random="
                + Math.round(Math.random() * 100);
        var param = {
            pwdType : pwdType,
            originPwd : originPwd,
        ...
        jQuery.post(url, param, function(data) {
            if (data.code == 0) {
                ...
                util.showerrortips(errorEle, data.msg);
                $("#" + phoneCodeEle).closest('.form-group').show();
            }
        }, "json");
    }
};
...
$(function() {
    ...
    $(".btn-sendmsg").on(
            ...
            function() {
                if (this.id == "unbindphone-newsendmessage") {
                    var areacode = $("#unbindphone-areaCode").val();
                    ...
                    var areacode = $("#bindphone-areaCode").val();
    ...
    $("#unbindloginpass-Btn").on("click", function() {
        security.submitPwdForm(0, 0);
    });
    $("#unbindtradepass-Btn").on("click", function() {
        security.submitPwdForm(1, 1);
    });
    $("#bindtradepass-Btn").on("click", function() {
        security.submitPwdForm(1, 2);
    });
});