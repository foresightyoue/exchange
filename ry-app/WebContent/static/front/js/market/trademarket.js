var market = {
	...
	loginUser : function(ele) {
		...
		if (login_acc == "") {
			util.layerTips("login_acc", "请输入账号", false, 3);
			return;
		}
		if (login_pwd == "") {
	...
	autoRefresh : function() {
		...
		var buysellcount = 20;
		var successcount = 60;
		var url = "/real/market2.html?symbol={0}&marketPrice="+marketPrice;
		...
		var tradesCallback = function(result) {
			...
			/*	$.each(data.trades, function(key, value) {
				...
				loghtml += '<span class="success-price ' + (rose < 0 ? "market-font-buy" : "market-font-sell") +'">'  + util.numFormat(Number(value.price), market.cnyDigit) + '</span>';
				loghtml += '<span class="success-amount">' + util.numFormat(Number(value.amount), market.coinDigit) + '</span>';
				loghtml += '</article>';
			});*/
		      var arr=[];
		      $.each(data.trades, function(key, value) {
		        var price_status= util.numFormat(Number(value.price), market.cnyDigit);
		        arr.push(price_status);
		        /*arr.reverse();*/
		        var rose = data.rose;
		        if (key >= successcount) {
		          return;
		        ...
		        loghtml += '<span class="success-price ' + ((arr[key] < arr[key-1] && key != 0) ? "market-font-buy" : "market-font-sell") +'">'  + util.numFormat(Number(value.price), market.cnyDigit) + '</span>';
		        loghtml += '<span class="success-amount">' + util.numFormat(Number(value.amount), market.coinDigit) + '</span>';
		        loghtml += '</article>';
			...
			//var p_new = util.numFormat(Number(data.pricenews[0].price), market.cnyDigit);
			var p_new = util.numFormat(data.p_new,market.cnyDigit)
...
//				$("#totalCoin").html(util.numFormat(result.data.totalCoin, market.coinDigit));
				...
				$.each(result.fentrusts1, function(key, value) {
					var dateTime=value.flastUpdatTime_s;
					Times=dateTime.substring(0,19);
					entrutsCur += '<article>';
					entrutsCur += '<span class="col-1">' + Times + '</span>';
				...
				$.each(result.fentrusts2, function(key, value) {
					var updateTime=value.flastUpdatTime_s;
					var dateTime=updateTime.substring(0,19);
					entrutsHis += '<article>';
					entrutsHis += '<span class="col-1">' + dateTime + '</span>';
