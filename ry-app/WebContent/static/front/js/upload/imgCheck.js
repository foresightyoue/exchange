	...
	if (checkFileType(fileId, 'img')) {
		...
		$.ajaxFileUpload({
	        ...
	        success: function (text, status) {
	            ...
	            if (data.code == 0) {
	            	if(fileId == "img1"){
	            		$("#path1").val(data.resultUrl);
	            		$("#echoImg1").prop("src", data.resultUrl);
	            	}
	            	if(fileId == "img2"){
	            		$("#path2").val(data.resultUrl);
	            		$("#echoImg2").prop("src", data.resultUrl);
	            	}
	            	if(fileId == "img3"){
	            		$("#path3").val(data.resultUrl);
	            		$("#echoImg3").prop("src", data.resultUrl);
	            	}
	            	layer.close(load);
	            }else if(data.code== 1){
	            	layer.msg(data.msg);
...
function checkFileType(inputId, fileType) {
    ...
    //图片校验
    if(fileType == "img"){
        if (ext != ".JPG" && ext != ".JPEG" && ext != ".PNG" && ext !=".BMP" && ext != ".APK" && ext !=".IPA") {
