    ...
    Response : function(marketFrom, type, coinVol, data) {
        ...
        if (ChartManager.getInstance().getChart()._money_type == 1) {
            ...
            for (var i in GLOBAL_VAR.KLineData) {
                ...
                // hank
                /*
                 * e[1] = parseFloat((e[1] * rate).toFixed(2)); e[2] =
                 * parseFloat((e[2] * rate).toFixed(2)); e[3] = parseFloat((e[3] *
                 * rate).toFixed(2)); e[4] = parseFloat((e[4] *
                 * rate).toFixed(2));
                 */
            }
        }
...
ChartManager.prototype.setCurrentDrawingTool = function(paramTool) {
    ...
    // "HoriStraightLine" // "HoriRayLine" // "HoriSegLine" //
    // "VertiStraightLine"
...
ChartManager.prototype.cleanDatas = function (dsName) {
    var ds = this.getDataSource(dsName);
    if (ds == undefined)
        return;
    ds.cleanDatas();
...
ChartManager.prototype.drawMain=function(frameName, context)
{
    drawn = false;
    /*
     * it = m_drawAreas.begin(); while (it != m_drawAreas.end()) { ChartArea*
     * pArea = getArea(*it); if (pArea != null && pArea.getFrameName() ==
     * frameName) { DrawArea(*pArea, g); drawn = true; it =
     * m_drawAreas.erase(it); } else { it++; } } it = m_drawDataSources.begin();
     * while (it != m_drawDataSources.end()) { if ((*it).find(frameName + ".") !=
     * 0) { it++; continue; } DataSource* pDS = getDataSource(*it); if (pDS !=
     * null) { DrawDataSource(*it, g); drawn = true; it =
     * m_drawDataSources.erase(it); } else { it = m_drawDataSources.erase(it);
     * return; } }
     */
    if (!drawn) {
...
/**
 * getIndicatorParameters 获取指标的ParamExpr数组
 * 
 * @param indicName
 * @returns {*}
 */
...
MainDataSource.prototype.getTime = function (date) {
    ...
    if(util.globalurl.isremote){
        ...
        etime=Math.round(new Date(timearr[0],parseInt(timearr[1])-1,timearr[2],timearr[3],timearr[4],timearr[5],timearr[6]).getTime());
    }
    return etime;
};
MainDataSource.prototype.cleanDatas = function () {
    this._dataItems = [] ;
};
MainDataSource.prototype.update = function (data) {
    ...
    if (len > 0) {
        ...
        for (i = 0; i < cnt; i++) {
            e = data[i];
            if (this.getTime(e[0]) == lastItem.date) {
...
var RequestData = function(showLoading) {
    ...
    $(document).ready(
        GLOBAL_VAR.G_HTTP_REQUEST = $.ajax({
            type        : "get",
            url         : util.globalurl.kline.format(util.globalurl.klineType[GLOBAL_VAR.time_type]),
            dataType    : 'text',
                ...
                if (GLOBAL_VAR.G_HTTP_REQUEST) {
                    ...
                    if (ChartManager.getInstance().getChart()._money_type == 1) {
                        ...
                        for (var i in GLOBAL_VAR.KLineData) {
                            // hank
                           /*
                             * var e = GLOBAL_VAR.KLineData[i]; e[1] =
                             * parseFloat((e[1] * rate).toFixed(2)); e[2] =
                             * parseFloat((e[2] * rate).toFixed(2)); e[3] =
                             * parseFloat((e[3] * rate).toFixed(2)); e[4] =
                             * parseFloat((e[4] * rate).toFixed(2));
                             */
                        }
                    }
                    try {
...
function switch_period(name) {
    ...
    if (name == 'line') {
        ...
        ChartSettings.save();
        GLOBAL_VAR.chartMgr.cleanDatas('frame0.k0');
        return;
    }
...
function getRealData(){
    ...
    if(util.globalurl.isremote){
        ...
        $.get(url,function(data){
            ...
            window.onPushingResponse(GLOBAL_VAR.mark_from,GLOBAL_VAR.time_type,GLOBAL_VAR.mark_from,tempdata);
        },"json");
        realtimeid=window.setTimeout(function() {
            getRealData();
        }, 2000);
    }else{
        url=util.globalurl.depth+"?"+GLOBAL_VAR.requestParam;
        $.get(url,function(data){
